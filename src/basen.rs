...
impl BaseN {
    ...
    pub fn to_usize(&self) -> Result<usize, &'static str> {
        ...
        for (i, x) in self.vec.iter().enumerate() {
            val10 += (*x as usize) * self.base.pow(i as u32);
        }
        Ok(val10)
    }
}
...
impl Mul for BaseN {
    ...
    fn mul(self, rhs: BaseN) -> Result<BaseN, &'static str> {
        ...
        BaseN::from_usize(self.base,
                          self.to_usize().unwrap() * rhs.to_usize().unwrap())
    }
}
